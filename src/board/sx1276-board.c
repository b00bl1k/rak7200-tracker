...
{
    static Gpio_t OscSelectPin;
    static Gpio_t TcxoOePin;
    bool IsOscCrystal;
    ...
    if( state == true )
    {
        ...
        GpioInit(&OscSelectPin, LORA_OSC_SEL, PIN_INPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0 );
        DelayMs( 10 );
        pin_state = GpioRead( &OscSelectPin );
        if ( pin_state == 0 )
        {
            // Low for TCXO
            IsOscCrystal = false;
        ...
        {
            // PC_1 High for Crystal
            IsOscCrystal = true;
        }

        if ( IsOscCrystal == false )
        {
            GpioInit(&TcxoOePin, LORA_TCXO_OE, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0 );
            DelayMs( 5 );
            GpioWrite( &TcxoOePin, 1);
            DelayMs( 10 );
